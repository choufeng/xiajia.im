...
- 环境变量已设：`VOLC_TTS_ACCESS_TOKEN`（火山豆包大模型 2.0 的 **API Key**，从「控制台 > API Key 管理」生成；注意与 v1 普通版 access token 不同体系）
...
- 鉴权失败（401/Invalid X-Api-Key）→ 提示用户检查 `VOLC_TTS_ACCESS_TOKEN` 是否为 v3 API Key
